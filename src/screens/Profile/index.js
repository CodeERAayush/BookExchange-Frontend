...
import { addMyBook, deleteBook } from '../../slices/ItemsSlice'
import ItemCard from '../../reusables/ItemCard'
import { add_item } from '../../slices/CartSlice'
import FastImage from 'react-native-fast-image'
import { showSuccess } from '../../constants/Gcconstant'
const Profile = ({ navigation }) => {
  ...
  const _deleteItem=(id)=>{
    const params = {
      url: `${API.API_BASEURL}/${API.DELETE_BOOK}`,
      method: 'DELETE',
      data:{
        bookId:id
      },
        ...
        <FlatList
          ...
          renderItem={({ item, index }) => (
            <ItemCard
              ...
              key={index}
              onPressDelete={()=>_deleteItem(item?._id)}
              onPressCart={() => {
